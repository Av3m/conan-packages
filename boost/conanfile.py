...
class BoostConan(ConanFile):
    ...
    url="https://github.com/Av3m/conan-boost"
    ...
    def source(self):
        ...
        zip_name = "%s.zip" % FOLDER_NAME if sys.platform == "win32" else "%s.tar.gz" % FOLDER_NAME
    ...
    def build(self):
        ...
        if self.settings.os == "Windows" and self.settings.compiler == "gcc":
            command += " mingw"
        try:
            self.run("cd %s && %s" % (FOLDER_NAME, command))
        except:
            self.run("cd %s && type bootstrap.log" % FOLDER_NAME
                     if self.settings.os == "Windows"
                     else "cd %s && cat bootstrap.log" % FOLDER_NAME)
            raise
...
#             ret += "-s ZLIB_BINARY=%s -s ZLIB_INCLUDE=%s -s ZLIB_LIBPATH=%s" % (lib_name, include_path, lib_path)
    ...
    def package(self):
        ...
        self.copy(pattern="*", dst="include/boost", src="%s/boost" % FOLDER_NAME)
        self.copy(pattern="*.a", dst="lib", src="%s/stage/lib" % FOLDER_NAME)
        self.copy(pattern="*.so", dst="lib", src="%s/stage/lib" % FOLDER_NAME)
        self.copy(pattern="*.so.*", dst="lib", src="%s/stage/lib" % FOLDER_NAME)
        self.copy(pattern="*.dylib*", dst="lib", src="%s/stage/lib" % FOLDER_NAME)
        self.copy(pattern="*.lib", dst="lib", src="%s/stage/lib" % FOLDER_NAME)
        self.copy(pattern="*.dll", dst="bin", src="%s/stage/lib" % FOLDER_NAME)
